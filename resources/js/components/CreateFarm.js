...
const ExpenseComponent = ({ crops }) => {
    ...
    const handleAddExpense = (e) => {
        ...
        if (expense.trim() !== "") {
            if (selectedCrop === "All crops") {
                crops.forEach((crop) => {
                    const newExpenses = cropExpenses[crop]
                        ? [...cropExpenses[crop], expense]
                        : [expense];
                ...
                const newExpenses = cropExpenses[selectedCrop]
                    ? [...cropExpenses[selectedCrop], expense]
                    : [expense];
                setCropExpenses({
                    ...cropExpenses,
    ...
    const handleDeleteEntry = (indexToRemove, crop) => {
        const updatedExpenses = cropExpenses[crop].filter(
            (_, index) => index !== indexToRemove
        );
        setCropExpenses({
            ...cropExpenses,
            ...
            <div className="box-cont p-3">
                ...
                <div className="row mt-3">
                    {crops.map((crop, index) => (
                        <div
                            key={index}
                            className="col-md-3 m-3 box-cont p-3 mb-3"
                        >
                            <div className="text-center ">
                                <h5>Expenses for {crop}</h5>
                            </div>
                            {cropExpenses[crop] &&
                                cropExpenses[crop].map((expense, idx) => (
